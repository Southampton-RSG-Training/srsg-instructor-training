---
title: Wrapping Up
teaching: 10
exercises: 5
---

::::::::::::::::::::::::::::::::::::::: objectives

- Reflect on the course.
- Articulate constructive feedback.

::::::::::::::::::::::::::::::::::::::::::::::::::

:::::::::::::::::::::::::::::::::::::::: questions

- What can we improve in this training?

::::::::::::::::::::::::::::::::::::::::::::::::::


TODO: needs revision, plus the following needs consideration:
TODO: - "Reward" - e.g. attendance certificate and/or higher-level certification process if they help out at a workshop
TODO: - A "culture of contribution", inc. how to improve our lessons, keep in touch
TODO: - Signpost next steps - help out at a workshop, carpentries/instructor training, etc.
TODO: - Next learning steps, inc. EoI for content creation workshop (half-day/full day)
TODO: - Link to survey

:::::::::::::::::::::::::::::::::::::::  challenge
  
Just as in our technical workshops, we collect feedback at the end of
Instructor Training. This will help us continually improve this training.

## One Up, One Down

10 mins.

Provide one up, one down feedback on the entire Instructor Training course. Remember:

- Say only one thing, and try not to duplicate. This gets harder for
  those who come later!
- Trainers should try not to respond, only record responses (e.g. in the
  shared document). This is also hard, but important!

::::::::::::::::::::::::::::::::::::::::::::::::::

:::::::::::::::::::::::::::::::::::::::  challenge

## Post Workshop Surveys

5 mins.

Assessment is very important to us! Please take the remaining time to
complete the post-training survey (your Trainers will provide the link).

::::::::::::::::::::::::::::::::::::::::::::::::::

## Thank You!


:::::::::::::::::::::::::::::::::::::::: keypoints

- Feedback applies to all kinds of learning, including learning how to teach.

::::::::::::::::::::::::::::::::::::::::::::::::::


